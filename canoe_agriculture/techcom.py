...
def build_technology_and_commodity_agri(
    ...
    comb_dict["Technology"] = pd.concat(
        [comb_dict["Technology"], tech_df], ignore_index=True
    )
    ...
    for i, com in enumerate(com_list):
        ...
        if code.startswith(sector_abv + "d_"):
            ...
            flag = "a"
            desc = f"Represents {desc_list[i]} in the agriculture sector"
        comm_rows.append(
            Commodity(name=code, flag=flag, description=desc, data_id=ids["CAN"])
        )
    comm_df = pd.DataFrame([row.model_dump(mode="python") for row in comm_rows])
    comb_dict["Commodity"] = pd.concat(
        [comb_dict["Commodity"], comm_df], ignore_index=True
    )

    comb_dict["__demand_com_list__"] = demand_com_list
    logger.info(
        "Technology rows: %d, Commodity rows: %d", len(tech_rows), len(comm_rows)
    )
    return comb_dict
